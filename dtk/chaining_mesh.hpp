...
namespace dtk{
  ...
  public:
    ...
    template<typename T>
    size_t get_cell_id_from_indexes_wrap(T* ijk);
    ...
    std::vector<size_t> get_neighbor_cell_id_list(size_t cell_index, size_t length);//not implmented
    std::vector<size_t> get_neighbor_cell_id_list(size_t* ijk, size_t length);//not implmented
    std::vector<size_t> get_neighbor_cell_id_list(float* xyz, size_t length);//not implmented
    std::vector<size_t> get_region_cell_id_list(size_t cell_index, size_t length);
    std::vector<size_t> get_region_cell_id_list(size_t* ijk, size_t length);
    std::vector<size_t> get_region_cell_id_list(float* xyz, float radius);//not implmented fully
    //vector based inputs
    std::vector<size_t> get_indexs_from_cell_id(size_t cell_id);
    
    //spacial query
    std::vector<size_t> query_elements_within(float* xyz, float radius);

    //utility
    float calculate_periodic_distance(float* xyz1, float *xyz);
    void move_points_together(float* target_xyz, float* moving_xyz);//not implemented
    void move_points_together(float* target_xyz, float** xyz, size_t size);//not implemented
    void copy_point(float** xyz, size_t index, float* xyz_dest);
  private:
  ...
  template<typename T>
  size_t ChainingMeshIndex::get_cell_id_from_indexes_wrap(T* ijk){
    size_t result;
