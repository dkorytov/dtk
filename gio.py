...
def gio_read(file_name,var_name,rank_num=-1):
    ...
    if(var_type==10):
        ...
        libpygio.read_gio_float(file_name.encode('utf-8'),var_name.encode('utf-8'),result.ctypes.data_as(ct.POINTER(ct.c_float)),rank_num)
        return result
    elif(var_type==1):
        #double
        result = np.ndarray(var_size,dtype=np.float64)
        libpygio.read_gio_double(file_name.encode('utf-8'),var_name.encode('utf-8'),result.ctypes.data_as(ct.POINTER(ct.c_double)),rank_num)
        return result
    elif(var_type==2):
        #int32
        result = np.ndarray(var_size,dtype=np.int32)
        libpygio.read_gio_int32(file_name.encode('utf-8'),var_name.encode('utf-8'),result.ctypes.data_as(ct.POINTER(ct.c_int32)),rank_num)
        return result
    elif(var_type==3):
        #int64
        result = np.ndarray(var_size,dtype=np.int64)
        libpygio.read_gio_int64(file_name.encode('utf-8'),var_name.encode('utf-8'),result.ctypes.data_as(ct.POINTER(ct.c_int64)),rank_num)
        return result        
    elif(var_type==4):
        #int8
        result = np.ndarray(var_size,dtype=np.int8)
        libpygio.read_gio_int8(file_name.encode('utf-8'),var_name.encode('utf-8'),result.ctypes.data_as(ct.POINTER(ct.c_int8)),rank_num)
        return result        
    elif(var_type==5):
        #int8
        result = np.ndarray(var_size,dtype=np.uint8)
        libpygio.read_gio_uint8(file_name.encode('utf-8'),var_name.encode('utf-8'),result.ctypes.data_as(ct.POINTER(ct.c_uint8)),rank_num)
        return result        
    else:
